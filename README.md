...
WhatsApp Web 工作台助手：AI 翻译、客户备注标签、快捷回复、语音转写、数据导出。本地存储，开源，不群发。
...
### 🌍 AI 智能翻译
- **来消息自动翻译**: 对方消息下方显示译文，可按联系人开关
- **智能译出**: 中文输入自动翻译成英文，预览后手动发送
- **翻译历史持久化**: 刷新页面后翻译结果保留 7 天
...
### 📇 客户档案管理
- **智能号码识别**: 自动显示国家、时区、当地时间
- **工作时间提示**: 判断对方是否在工作时间（09:00-18:00）
- **手动修正**: 号码识别错误时可手动输入
- **本地备注**: 记录客户信息、跟进事项（本机存储）
- **颜色标签**: 自定义标签分类客户（如 VIP、待跟进）

### 🎙️ 语音转写
- 点击语音消息自动转写文字（OpenAI Whisper）
- 转写结果可再次翻译

### 💾 数据管理
- **导出备份**: 一键导出客户档案、快捷回复、标签到 JSON
- **导入恢复**: 从备份文件导入，智能去重合并
- **跨设备同步**: 手动迁移数据到其他电脑
- **安全保障**: 导出文件不包含 API Key
